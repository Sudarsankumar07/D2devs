      ...
      <div className="mb-12 flex flex-wrap gap-3 border-b border-[#c3c6d7]/30 pb-4">
        {categories.map((cat) => (
          <button
            ...
            className={`px-4 py-2 rounded-lg font-label-technical text-xs transition-all cursor-pointer ${
            ...
            >
              ...
              <div className="relative z-20 w-full h-full p-6 sm:p-8 flex flex-col justify-end overflow-hidden">
                <div className="flex gap-3 items-center mb-4">
                  ...
                  <span className={`font-label-technical font-bold tracking-wide text-xs sm:text-sm transition-colors duration-500 ${isActive ? 'text-[#40b8ff]' : 'text-white/60'}`}>
                    {project.categoryLabel}
                  </span>
                </div>

                <div className="overflow-hidden">
                  <h3 className={`font-sans font-bold text-white tracking-tight transition-all duration-700 ${isActive ? 'text-2xl sm:text-4xl lg:text-5xl mb-4 whitespace-normal' : 'text-xl whitespace-nowrap overflow-hidden text-ellipsis mb-0'}`}>
                    {project.title}
                  </h3>
                  
                  <div className={`transition-all duration-700 ease-in-out ${isActive ? 'opacity-100 max-h-48' : 'opacity-0 max-h-0'}`}>
                    <p className="font-sans text-white/80 text-sm sm:text-base max-w-xl leading-relaxed mb-6">
                      {project.description}
                    </p>
